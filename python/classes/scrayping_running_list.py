...
class Main():
  ...
  def scrayping_running_list(self,race_id):
    ...
    # クラス名で指定して要素を取得
    tr = self.driver.find_elements(By.CLASS_NAME, "HorseList")
    ...
    for x in range(0, len(tr)):
      ...
      horse_id = self.driver.find_elements(By.CLASS_NAME, "HorseName")[x+1].find_element(By.TAG_NAME,"a").get_attribute("href").split("horse/")[1]
      jockey_id = self.driver.find_elements(By.CLASS_NAME, "Jockey")[x+1].find_element(By.TAG_NAME,"a").get_attribute("href").split("recent/")[1].replace("/","")
      trainer_id = self.driver.find_elements(By.CLASS_NAME, "Trainer")[x+1].find_element(By.TAG_NAME,"a").get_attribute("href").split("recent/")[1].replace("/","")
