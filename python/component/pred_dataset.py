...
def entryHorse(raceinfo, horseinfo):
    #pickleデータロード
    dfRaceInfo = pd.read_pickle('./data/race_info.pkl')
    dfHorseResult = pd.read_pickle('./data/horse_result2.pkl')
    dfHorsePed = pd.read_pickle('./data/horse_ped2.pkl')
    #出馬表とレース情報テーブル結合
    ...
    dfEntryp = pd.merge(left=df, right=dfHorsePed, how='left', on='horseId')
    ...
    tmpdf = df.copy()
    #着順から正解列を生成
    tmpdf['Accu'] = tmpdf['着順'].map(lambda x: 1 if x <= 3 else 0)
    #データセットを学習データをする
    train = tmpdf
...
#出馬表のラベルエンコーディング
    #ラベルエンコーディング関数の定義
    def labelEncode2(df, target, le, recflg=False):
        #複数列のラベルエンコーディング関数の定義
        def listEncoder2(tdf, le, cols):
            #データフレームのコピー
            tdf_ = tdf.copy()
            #列名から値を取り出す
            encoList = []
        ...
        if not recflg:
            ...
            if type(target) != list:
                ...
                #戦績以外で複数データだったら複数列エンコードの実行
                tdf, le = listEncoder2(tdf, le, target)
        else:
            #戦績データは列名にサフィックスを付与したリストを生成
            cols9 = [target] + [target + '_' + str(i) for i in range(1, 10)]
            #複数列エンコードの実行
            tdf, le = listEncoder2(tdf, le, cols9)
        #データフレームとエンコーダーをreeturn
        return tdf
    ...
    print()
    print(df)
    df = labelEncode2(df,horseList,leHorse)
    df = labelEncode2(df,'性',leGender)
    df = labelEncode2(df,'trainerId',leTrainer)
    df = labelEncode2(df,'拠点',leHomeBase)
    df = labelEncode2(df,'jockeyId',leJockey,recflg=True)
    df = labelEncode2(df,'ハンデ',leHandi,recflg=True)
    df = labelEncode2(df,'コース種',leType,recflg=True)
    df = labelEncode2(df,'コース回り',leDir,recflg=True)
    df = labelEncode2(df,'天気',leWether,recflg=True)
    df = labelEncode2(df,'馬場',leCondition,recflg=True)
    df = labelEncode2(df,'開催場所',lePlace,recflg=True)
    df = labelEncode2(df,'グレード',leGrade,recflg=True)
    df = labelEncode2(df,'制限',leRegulation,recflg=True)
    #量的変数の列名を生成
    numericCols = ['年齢']
    cols1 = ['枠番','馬番','単勝','人気','斤量','馬体重',
            '体重増減','出走間隔','R','距離']
    ...
    view_df = eval_df[['raceId','馬名','馬番','枠番','単勝','人気','着順','予測着順',
