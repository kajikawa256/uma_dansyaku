# 毎日8時に自動実行するファイル(python内のcronによる自動実行)
...
#command_first = 'cd / && python python/cron/confirm_info.py'
# command_second = 'python ./cron/just_before.py'
schedule = 'm h * * *'
...
if len(raceIdList) != 0:
  cron_instans.monitor_start(len(raceIdList))
